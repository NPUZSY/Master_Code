...
from Scripts.Chapter5.Meta_RL_Engine import ResultSaver, create_output_dir, RuleBasedPolicy, DPStrategy, MetaRLPolicy, load_model
...
# ====================== 测试函数 ======================
def test_algorithm(algorithm_name, agent_list, env, max_steps=1000, policy=None):
    """
    测试指定算法在环境中的表现
    ...
    Args:
        ...
        env: 测试环境
        max_steps: 最大测试步数
        policy: MetaRLPolicy对象，用于Meta-RL算法
    ...
    while steps < max_steps:
        ...
        if algorithm_name == "Joint_Net":
            ...
            a_fc = torch.argmax(fc_action_out, dim=1).item()
            a_bat = torch.argmax(bat_action_out, dim=1).item()
            a_sc = torch.argmax(sc_action_out, dim=1).item()
        else:
            # 其他算法
        ...
        # 计算跟踪误差
        P_load = float(state[0])
        P_fc = float(next_state[2])
        tracking_error = abs(P_load - P_fc)
    ...
    power_data = {
        ...
        'soc_bat': soc_bat,
        'soc_sc': soc_sc,
        'temperature': [env.temperature[0]] * steps  # 假设环境温度恒定
    }
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='Chapter5最终对比实验脚本')
    parser.add_argument('--meta-rl-model-path', type=str, 
                        default='/home/siyu/Master_Code/nets/Chap5/fc_tracking_test/fc_tracking_model_best.pth',
                        help='Meta-RL模型路径')
    ...
    if not args.output_dir:
        output_dir = create_output_dir("final_comparison")
    else:
    ...
    # 加载Meta-RL模型
    meta_rl_policy = MetaRLPolicy(hidden_dim=512)
    if not load_model(meta_rl_policy, args.meta_rl_model_path):
        print("❌ 无法加载Meta-RL模型，使用默认模型")
    else:
        print(f"✅ 成功加载Meta-RL模型: {args.meta_rl_model_path}")
    meta_rl_policy.eval()
    
    # 初始化算法列表
    algorithms = {
        "Meta_RL": (None, meta_rl_policy),
        "Joint_Net": ([fc_agent, bat_agent, sc_agent], None),
        "Rule_Based": (RuleBasedPolicy(), None),
        "DP": (DPStrategy(), None)
    }
    ...
    all_scenarios_results = {
        "config": {
            "meta_rl_model_path": args.meta_rl_model_path,
    ...
    for scenario in test_scenarios:
        ...
        # 测试每种算法
        for algo_name, (agent, policy) in algorithms.items():
            print(f"\n🔍 测试算法: {algo_name}")
            
            # 测试算法
            power_data, performance = test_algorithm(algo_name, agent, env, max_steps=args.max_steps, policy=policy)
            ...
            print(f"   平均跟踪误差: {performance['average_tracking_error']:.4f} W")
        
        # 保存场景结果
        all_scenarios_results["scenarios"][scenario] = scenario_results
    
    # 保存所有测试结果
    result_saver.save_results_json(all_scenarios_results, "final_comparison_results.json")
